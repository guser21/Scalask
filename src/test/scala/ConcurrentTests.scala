...
class ConcurrentTests extends FunSuite with BeforeAndAfterEach {
  ...
  def putDeleteRemove(segList: SegmentList, times: Int = 100 * 1000) = (1 to times).par.foreach {
    case i if i % 3 == 0 => segList.put("0", s"$i")
    case i if i % 3 == 1 => {
      segList.get("0"); Thread.sleep(10); println("got the 0 value")
    }
    case i if i % 3 == 2 => {
      segList.remove("0"); Thread.sleep(10); println("removed ")
    }
  }
